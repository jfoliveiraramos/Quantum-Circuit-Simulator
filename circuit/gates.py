...
class GateBuilder:
    ...
    def build(self, gate: Gate, qubit: int) -> Gate:
        qubit = self.dim - 1 - qubit
        mats = [gate if i == qubit else gates.I for i in range(self._dim)]
    ...
    def build_ctrl(self, gate: Gate, ctrl: int, target: int) -> Gate:
        ctrl = self.dim - 1 - ctrl
        target = self.dim - 1 - target

        zero_proj = sp.csr_matrix([[1, 0], [0, 0]], dtype=np.complex128)
        one_proj = sp.csr_matrix([[0, 0], [0, 1]], dtype=np.complex128)

        zero_mats = [zero_proj if i == ctrl else gates.I for i in range(self._dim)]
        one_mats = [
            one_proj if i == ctrl else gate if i == target else gates.I
